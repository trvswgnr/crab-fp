pub mod utilities {
    ...
    where
        A: Clone + 'static, // we need to be able to clone so that we can use the same value multiple times
        B: 'static,
        C: 'static,
    {
        move |a: A| Box::new(move |b: B| f(a.clone(), b))
    }
...
#[cfg(test)]
pub(crate) mod for_testing {
    #[cfg(feature = "no_std")]
    use crate::fixed_string::{String, ToString};
